...
- [x] Real Self verification (on-chain read; delete `setTimeout` fake). `(feat(frontend): real self)` — `SelfProtocolService.waitForVerification` now polls `registryService.isSelfVerified` (real `isFullyVerified` read) instead of two chained `setTimeout`s always resolving success; `checkVerificationStatus` is a real read too, not a hardcoded `false`.
- [x] E2E in-browser vs testnet: register → prove → verify → claim. `(test(frontend): e2e)` — vs a local Anvil devnet, not a public testnet (Phase 1e's job); nothing was deployed anywhere public yet, so a real testnet run wasn't possible. `frontend/e2e/anvil-e2e.js` drives real Chrome against the real UI: register (real Poseidon commitment, real escrow tx) → wait out inactivity → trigger grace (new UI button — nothing exposed this before) → wait out grace → generate a real UltraHonk proof in-browser → executeWill (real on-chain verify) → claim (real Merkle proof, real ETH transfer). `contracts/script/DeployLocalE2E.s.sol` deploys the real stack (MockUSDC/MockSelfVerifier stand in for the real ERC20/Self hub only). Getting this to actually pass surfaced and fixed real bugs that predate this session, not just test-harness issues:
  - `registryService.getChain(31337)` returned viem's `localhost` preset, which has chain id **1337**, not 31337 — every local-chain write was broken (wrong-chain error from viem). Now uses `hardhat` (id 31337).
  - `waitForTransaction` never checked `receipt.status`, so a reverted tx read as success everywhere (register/checkIn/triggerGracePeriod/veto/executeWill/claim) — masked the two bugs below during earlier phases' manual smoke-testing. Now throws on `status !== "success"`.
  - `register`/`execute` fed human decimal ETH/USDC strings (e.g. `"0.01"`) straight into the circuit witness; `noirService.stringToBigInt` only parses plain integers, so any decimal amount silently hashed the *text* instead of converting it, corrupting the circuit's declared `total_eth`/`total_usdc` against the real escrowed wei/base-units. Execute-will was broken for any non-integer amount — i.e. almost always. Fixed by converting through `parseEther`/`parseUnits` before building the witness, in both `register/page.tsx` and `execute/page.tsx`.
