...
- [ ] E2E in-browser vs testnet: register → prove → verify → claim. `(test(frontend): e2e)` — **blocked on Phase 1e**: nothing is deployed yet (all addresses are the zero-address placeholder), so there is no live registry/verifier/token to exercise. Revisit once Phase 1e deploys to a real testnet.
