...
contract SelfHumanVerifier is SelfVerificationRoot, Ownable {
    ...
    function isAgeValid(
        address userAddress
    ) external view returns (bool ageValid) {
    ...
    function isFullyVerified(
        address userAddress
    ) external view returns (bool fullyVerified) {
    ...
    function getContractStats()
        external
        view
        returns (uint256 totalUsers, uint256 minimumAge, bytes32 configId)
    {
        return (totalVerifiedUsers, MINIMUM_AGE, verificationConfigId);
    }
    ...
    function batchCheckVerification(
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            results[i] =
                verifiedHumans[addresses[i]] &&
                ageVerified[addresses[i]];
        }
    }
}
